...
ACTIVE_SPIN_LEADERBOARD = "https://baseballsavant.mlb.com/leaderboard/active-spin"
...
PITCH_CODE_TO_NAME = {
    ...
    "EP": "Eephus", "PO": "Pitch Out", "FA": "Fastball", "SV": "Slurve",
    "CS": "Slow Curve",
}
...
def canonical_pitch_key(value: Any) -> str:
    v = str(value or "").strip()
    code = v.upper()
    if code in PITCH_CODE_TO_NAME:
        return norm(PITCH_CODE_TO_NAME[code])
    return norm(v)
...
def ambient(s: requests.Session, row: pd.Series, raw: Path, health: Health) -> dict[str, Any]:
    game_dt = pd.to_datetime(row.game_datetime_utc, utc=True)
    days_ahead = (game_dt.date() - datetime.now(timezone.utc).date()).days
    ...
    if len(ts) == 0:
        return {}
    i = int(np.argmin(abs(ts - game_dt)))
    delta_minutes = float(abs((ts[i] - game_dt).total_seconds()) / 60.0)
    if delta_minutes > 90:
        health.add("WARN", "AMBIENT_FORECAST_TIME_GAP", row.game_pk,
                    f"Nearest available forecast hour is {delta_minutes:.0f} minutes from first pitch "
                    f"(selected={ts[i]}, game={game_dt}); conditions may not reflect actual game-time weather.",
                    "Confirm Open-Meteo forecast_days window covers the requested date/venue; verify --date argument.")
    try:
        ...
        return {"ambient_temperature_c": tc, "ambient_relative_humidity_pct": rh,
                ...
                "forecast_index_selected": i, "forecast_time_delta_minutes": round(delta_minutes, 1)}
    except Exception as e:
...
def outdoor_branch(row: pd.Series, a: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        **w, "wind_model_status": "APPLICABLE_OUTDOOR",
        ...
        "forecast_time_delta_minutes": a.get("forecast_time_delta_minutes"),
    }
...
def resolve_roof_state(s: requests.Session, row: pd.Series, raw: Path, health: Health, cache: dict[str, Any],
    ...
    conflict_rows.append({
        ...
        "total_votes_cast": len(all_nonnull),
    })
    ...
    if len(all_nonnull) == 1:
        ...
        return "UNRESOLVED_DUAL_BRANCH", "SINGLE_SOURCE_INSUFFICIENT_QUORUM", votes
    ...
    return "UNRESOLVED_DUAL_BRANCH", "NO_DECISIVE_SOURCE_TWO_BRANCH_MODEL", votes
...
def _leaderboard_full_name_series(d: pd.DataFrame) -> pd.Series:
    name_col = first(d, "player_name", "name")
    if name_col:
        def flip(v: Any) -> str:
            v = str(v or "")
            if "," in v:
                last, first_ = [x.strip() for x in v.split(",", 1)]
                return norm(f"{first_} {last}")
            return norm(v)
        return d[name_col].map(flip)
    last_col = first(d, "last_name")
    first_col = first(d, "first_name")
    if last_col and first_col:
        return (d[first_col].astype(str) + " " + d[last_col].astype(str)).map(norm)
    return pd.Series([""] * len(d), index=d.index)


def fetch_active_spin_leaderboard(s: requests.Session, year: int, raw: Path, health: Health) -> pd.DataFrame:
    """Fetches Baseball Savant's season Active Spin leaderboard -- the same endpoint
    pybaseball.statcast_pitcher_active_spin uses in production. Active spin is NOT
    part of the per-pitch statcast_search/csv export (verified against
    https://baseballsavant.mlb.com/csv-docs); it only exists on this leaderboard."""
    url = f"{ACTIVE_SPIN_LEADERBOARD}?year={year}_spin-based&min=1&hand=&csv=true"
    try:
        r = s.get(url, timeout=45)
        r.raise_for_status()
        p = raw / f"active_spin_leaderboard_{year}.csv"
        p.write_bytes(r.content)
        d = pd.read_csv(p)
        if d.empty:
            health.add("WARN", "ACTIVE_SPIN_LEADERBOARD_EMPTY", year, "Leaderboard returned zero rows.",
                        "Active spin will be unavailable this run; pitch physics still computed without it.")
            return pd.DataFrame()
        d.columns = [norm(x).replace(" ", "_") for x in d.columns]
        d["_full_name_norm"] = _leaderboard_full_name_series(d)
        pitch_col = first(d, "pitch_type_name", "pitch_name", "pitch_type")
        if not pitch_col:
            health.add("WARN", "ACTIVE_SPIN_LEADERBOARD_NO_PITCH_COLUMN", year, f"columns={list(d.columns)}",
                        "Cannot key active spin by pitch type this run.")
            return pd.DataFrame()
        d["_pitch_key"] = d[pitch_col].map(canonical_pitch_key)
        return d
    except Exception as e:
        health.add("WARN", "ACTIVE_SPIN_LEADERBOARD_FETCH_FAILED", year, f"{type(e).__name__}: {e}",
                    "Active spin will be unavailable this run; pitch physics still computed without it.")
        return pd.DataFrame()


def merge_active_spin_into_arsenal(arsenal: pd.DataFrame, pitcher_id: Any, pitcher_name: str,
                                    leaderboard: pd.DataFrame, merge_audit_rows: list[dict[str, Any]],
                                    year: int) -> pd.DataFrame:
    out = arsenal.copy()
    if out.empty:
        return out
    out["active_spin_pct"] = np.nan
    out["active_spin_source"] = "NO_LEADERBOARD_DATA"

    if leaderboard is None or leaderboard.empty:
        for _, row in out.iterrows():
            merge_audit_rows.append({"pitcher_id": pitcher_id, "pitcher_name": pitcher_name,
                                      "pitch_type": row.get("pitch_type"), "match_method": "LEADERBOARD_UNAVAILABLE",
                                      "active_spin_pct": None, "leaderboard_year": year})
        return out

    id_col = first(leaderboard, "player_id", "pitcher_id", "mlbam_id")
    candidates_by_id = pd.DataFrame()
    if id_col and pd.notna(pitcher_id):
        candidates_by_id = leaderboard[num(leaderboard[id_col]).eq(int(pitcher_id))]
    name_key = norm(pitcher_name) if pitcher_name else ""
    candidates_by_name = leaderboard[leaderboard["_full_name_norm"].eq(name_key)] if name_key else pd.DataFrame()

    for idx, row in out.iterrows():
        pkey = canonical_pitch_key(row.get("pitch_type"))
        if not candidates_by_id.empty:
            pool, method = candidates_by_id, "PLAYER_ID_MATCH"
        elif not candidates_by_name.empty:
            pool, method = candidates_by_name, "NAME_MATCH"
        else:
            pool, method = pd.DataFrame(), "NO_PITCHER_MATCH"

        hit = pool[pool["_pitch_key"].eq(pkey)] if not pool.empty else pd.DataFrame()
        if not hit.empty:
            active_field = first(hit, "active_spin_pct", "pct_active_spin", "active_spin_percent", "active_spin")
            val = float(num(hit.iloc[0][active_field])) if active_field else np.nan
            out.at[idx, "active_spin_pct"] = val
            out.at[idx, "active_spin_source"] = f"SAVANT_ACTIVE_SPIN_LEADERBOARD_{year}"
            merge_audit_rows.append({"pitcher_id": pitcher_id, "pitcher_name": pitcher_name,
                                      "pitch_type": row.get("pitch_type"), "match_method": method,
                                      "active_spin_pct": val, "leaderboard_year": year})
        else:
            final_method = method if method == "NO_PITCHER_MATCH" else f"{method}_NO_PITCH_TYPE_HIT"
            merge_audit_rows.append({"pitcher_id": pitcher_id, "pitcher_name": pitcher_name,
                                      "pitch_type": row.get("pitch_type"), "match_method": final_method,
                                      "active_spin_pct": None, "leaderboard_year": year})
    return out
...
def summarize_window(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if df.empty:
        return pd.DataFrame()
    keep_num = [c for c in ["release_speed", "effective_speed", "release_spin_rate", "spin_axis",
                             "pfx_x", "pfx_z", "release_extension"] if c in df.columns]
    for c in keep_num:
        df[c] = num(df[c])
...
def weighted_arsenal(season_df: pd.DataFrame, recent_df: pd.DataFrame, recent_weight: float = 0.65) -> pd.DataFrame:
    ...
    for metric in ["pitches", "release_speed", "effective_speed", "release_spin_rate", "spin_axis",
                    "pfx_x", "pfx_z", "release_extension", "usage_pct"]:
...
def apply_environment_physics(arsenal: pd.DataFrame, branch: dict[str, Any]) -> pd.DataFrame:
    ...
    if "active_spin_pct" in out.columns:
        out["active_spin_environment_index"] = out["active_spin_pct"] * ratio
...
GLOSSARY: list[dict[str, str]] = [
    ...
    {"section": "game_environment_audit", "field": "forecast_time_delta_minutes", "meaning": "Minutes between the Open-Meteo forecast hour actually selected and the game's real first-pitch time. Large values mean the displayed weather may not reflect real game-time conditions.", "derivation": "abs(selected_forecast_hour_utc - game_datetime_utc) in minutes", "unit": "minutes"},
    ...
    {"section": "pitch_physics_environment", "field": "active_spin_pct", "meaning": "Percent of total spin contributing to Magnus-force movement, sourced from Baseball Savant's SEASON Active Spin leaderboard -- NOT the per-pitch Statcast Search export, which does not contain this field at all. Matched to this pitcher+pitch-type by MLBAM player_id first, normalized full name as fallback.", "derivation": "baseballsavant.mlb.com/leaderboard/active-spin CSV export, joined by player_id/name + pitch-type key (see active_spin_merge_audit.csv for the match trail)", "unit": "%"},
    ...
    {"section": "active_spin_merge_audit", "field": "match_method", "meaning": "How this pitcher+pitch-type row was matched against the Active Spin leaderboard: PLAYER_ID_MATCH (most reliable), NAME_MATCH (fallback), or a *_NO_PITCH_TYPE_HIT / NO_PITCHER_MATCH / LEADERBOARD_UNAVAILABLE failure reason.", "derivation": "merge_active_spin_into_arsenal()", "unit": "categorical"},
]
...
def validate_active_spin_coverage(pitch_rows_df: pd.DataFrame, leaderboard_fetched_ok: bool, health: Health) -> None:
    if pitch_rows_df.empty or not leaderboard_fetched_ok:
        return
    if "active_spin_pct" not in pitch_rows_df.columns:
        health.add("FATAL", "ACTIVE_SPIN_COLUMN_MISSING", "run",
                    "pitch_physics rows exist but active_spin_pct column was never populated on the arsenal frame.",
                    "Check merge_active_spin_into_arsenal() call site in main().")
        return
    non_null = pitch_rows_df["active_spin_pct"].notna().sum()
    if non_null == 0:
        health.add("FATAL", "ACTIVE_SPIN_ZERO_MATCH_RATE", "run",
                    f"Active Spin leaderboard fetched successfully but 0 of {len(pitch_rows_df)} pitch rows matched any record.",
                    "Investigate name/pitch-type key normalization in merge_active_spin_into_arsenal(); check active_spin_merge_audit.csv.")


def copy_dashboard_and_glossary(run_dir: Path, script_dir: Path) -> None:
    src = script_dir / "mlb-pitch-environment-live-dashboard-V53.html"
    if src.exists():
        shutil.copy2(src, run_dir / "mlb-pitch-environment-live-dashboard-V53.html")
    (run_dir / "V53_IN_APP_GLOSSARY.json").write_text(json.dumps(GLOSSARY, indent=2), encoding="utf-8")
...
def main() -> None:
    ...
    leaderboard_year = pd.to_datetime(args.date).year
    active_spin_board = fetch_active_spin_leaderboard(s, leaderboard_year, raw, health)
    leaderboard_fetched_ok = not active_spin_board.empty
    active_spin_merge_rows: list[dict[str, Any]] = []
    ...
    for _, g in games_unique.iterrows():
        ...
        for b in branches:
            ...
            provenance_rows.append({
                ...
                "forecast_valid_time_est": utc_to_est_str(b.get("forecast_valid_time_utc")),
                "forecast_time_delta_minutes": b.get("forecast_time_delta_minutes"),
    ...
    for _, r in joined.iterrows():
        ...
        if arsenal.empty:
            continue
        arsenal = merge_active_spin_into_arsenal(arsenal, int(r.pitcher_id), r.pitcher_name,
                                                  active_spin_board, active_spin_merge_rows, leaderboard_year)
    ...
    pitch_rows_df = pd.DataFrame(pitch_rows)
    validate_active_spin_coverage(pitch_rows_df, leaderboard_fetched_ok, health)
    ...
    write_csv_contract(game_env, run_dir / "game_environment_audit.csv",
        ["game_pk", "game_datetime_utc", "game_datetime_est", "venue_name_api", "venue_name", "roofclass",
         ...
         "wind_speed_mph", "wind_park_relative_description", "forecast_valid_time_est", "forecast_time_delta_minutes"])
    ...
    write_csv_contract(pd.DataFrame(roof_conflict_rows), run_dir / "roof_conflict_audit.csv",
        ["game_pk", "venue_name", "dedicated_votes_json", "generic_votes_json", "has_conflict", "resolution", "total_votes_cast"])
    ...
    write_csv_contract(pitch_rows_df, run_dir / "pitch_physics_environment.csv",
        ["game_pk", "side", "team", "pitcher_id", "pitcher_name", "pitcher_resolution_method", "venue_name",
         ...
         "release_speed", "effective_speed", "release_spin_rate", "spin_axis", "active_spin_pct", "active_spin_source",
         "release_extension", "pfx_x", "pfx_z", "projected_pfx_x", "delta_pfx_x_inches",
    ...
    write_csv_contract(pd.DataFrame(provenance_rows), run_dir / "atmosphere_provenance_audit.csv",
        ["game_pk", "venue_name", "branch", "temperature_source", "pressure_source", "forecast_valid_time_utc",
         "forecast_valid_time_est", "forecast_time_delta_minutes", "venue_lat", "venue_lon", "venue_elevation_m"])
    ...
    write_csv_contract(pd.DataFrame(active_spin_merge_rows), run_dir / "active_spin_merge_audit.csv",
    ...
    active_spin_match_rate = (100.0 * pitch_rows_df["active_spin_pct"].notna().sum() / len(pitch_rows_df)) if len(pitch_rows_df) else float("nan")
    diag_rows = [
        ...
        {"check": "active_spin_leaderboard_fetched", "value": float(1 if leaderboard_fetched_ok else 0),
         "status": "PASS" if leaderboard_fetched_ok else "WARN_LEADERBOARD_UNAVAILABLE"},
        {"check": "active_spin_match_rate_pct", "value": float(active_spin_match_rate) if not math.isnan(active_spin_match_rate) else float("nan"),
         "status": "PASS" if (not math.isnan(active_spin_match_rate) and active_spin_match_rate > 0) else "WARN_NO_ACTIVE_SPIN_MATCHES"},
    ...
    print(json.dumps({
        ...
        "active_spin_leaderboard_fetched": leaderboard_fetched_ok,
        "active_spin_match_rate_pct": None if math.isnan(active_spin_match_rate) else round(active_spin_match_rate, 1),
