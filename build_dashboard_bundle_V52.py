#!/usr/bin/env python3
"""V52 strict dashboard-bundle builder.

Consumes the V52 runner's exact output contract, including the newly expanded
pitch_physics_environment columns (active_spin, spin_axis, delta_pfx, etc.) and
the new display fields (temperature_f, elevation_ft, wind_speed_mph, EST times).
Blocks packaging if a required artifact is missing/unexpectedly empty or if
pitch-physics/decomposition row counts don't match.
"""
...
REQUIRED = {
    "game_environment_audit": ("game_environment_audit.csv",
        ["game_pk", "venue_name", "roofclass", "roof_state", "branch", "air_density_kg_m3"]),
    ...
    "pitch_physics_environment": ("pitch_physics_environment.csv",
        ["game_pk", "pitcher_id", "pitch_type", "branch"]),
    "atmosphere_provenance_audit": ("atmosphere_provenance_audit.csv",
        ["game_pk", "branch", "temperature_source", "pressure_source"]),
...
OPTIONAL_EMPTY_OK = {"roof_conflict_audit", "roof_evidence_audit", "findings"}
...
def main() -> int:
    ...
    glossary = []
    gp = args.glossary or (run / "V52_IN_APP_GLOSSARY.json")
    if gp.exists():
        try:
